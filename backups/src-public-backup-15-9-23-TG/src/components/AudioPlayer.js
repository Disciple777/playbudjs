...
function AudioPlayer({ audioFiles, currentIndex }) {
  ...
  useEffect(() => {
    // Reset the play button when currentIndex changes
    setIsPlaying(false);
    
    // Ensure that audioRef.current is not null
    if (audioRef.current && audioFiles.length > 0) {
      audioRef.current.src = audioFiles[currentIndex].data;
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(updateCurrentPosition, 200); // Update every second
  ...
  return (
    <div className="audio-player">
      ...
      <div className="file-info">
        {audioFiles.length > 0 && (
          <p>Selected File: {audioFiles[currentIndex].name}</p>
        )}
      </div>
